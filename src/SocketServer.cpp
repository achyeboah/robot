...
namespace exploringRPi {
	...
	int SocketServer::send(std::string message){
		...
		// make sure a broken pipe does not terminate our session
		// signal(SIGPIPE, pipeBrokenHandler);
		signal(SIGPIPE, SIG_IGN);

		int n = 0;
		if (clientConnected == true)
			n = write(this->clientSocketfd, writeBuffer, length+1);
	...
	void SocketServer::getClient(char* client_ip, unsigned int& client_port) const{
		client_ip = inet_ntoa(this->clientAddress.sin_addr);
		client_port = ntohs(this->clientAddress.sin_port);
	}

	bool SocketServer::getConnected(void) const{
