...
namespace samsRobot{
	...
	unsigned int read_imu_data(const char* filename, imu_data& data){
		...
		fscanf(fhandle, "%d: %f %f %f; %f %f %f; %f %f", &(data.type), &(data.pitch), &(data.yaw), &(data.roll), &(data.wVelX), &(data.wVelY), &(data.wVelZ), &(data.head), &(data.temp));
		// fscanf(fhandle, "%d: %f %f %f; %f %f %f; %f %f", &(type), &(pitch), &(yaw), &(roll), &(wVelX), &(wVelY), &(wVelZ), &(heading), &(temp));
	...
	unsigned int write_imu_data(const char* filename, const imu_data data){
		...
		fprintf(fhandle, "%d: %f %f %f; %f %f %f; %f %f\n",
